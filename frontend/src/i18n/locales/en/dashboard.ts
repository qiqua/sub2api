export default {
  ...
  keys: {
    ...
    autoRouting: {
      title: 'Multi-group load balancing',
      description: 'When enabled, this API key keeps the primary group as its anchor and automatically selects the least loaded usable group from same-platform candidates. Users still call with this single key.',
      candidateHint: 'Select same-platform candidate groups. The primary group is always included automatically.',
      noCandidates: 'No same-platform candidate groups are available for the selected primary group',
      groupRequired: 'Please select at least one candidate group when multi-group load balancing is enabled',
      selectedHint: '{count} candidate groups selected. Requests will be balanced across {total} groups in total.',
      badge: 'Auto {count} groups',
      badgeTitle: 'This API key has multi-group automatic routing enabled'
    },
    usage: 'Usage',
    today: 'Today',
    total: 'Last 30d',
