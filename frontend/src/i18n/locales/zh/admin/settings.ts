    ...
    settings: {
      ...
      features: {
        scheduledAccountTests: {
          title: '自动定时账号测试',
          description: '控制定时账号测试计划是否自动运行。关闭后不会在启动或后台自动扫描账号；手动批量健康检查不受影响。',
          enabled: '启用自动定时账号测试',
          enabledHint: '2C2G 或账号数量较多的部署建议保持关闭，避免重启后大量到期计划同时运行导致 CPU 峰值。',
        },
        channelMonitor: {
          title: '渠道监控',
      ...
      scheduling: {
        ...
        openaiFirstOutput: {
          title: 'OpenAI 首字 / 流式超时',
          description: '这些值保存在数据库中，保存后立即生效。0 表示关闭限制。',
          firstOutputTimeout: '普通请求首字总预算（秒）',
          firstOutputTimeoutHint: '首个语义输出前的总预算。0=关闭；建议 20-25。',
          highEffortFirstOutputTimeout: '高推理请求首字总预算（秒）',
          highEffortFirstOutputTimeoutHint: 'high / xhigh / max 请求使用更长预算。0=跟随普通预算；建议 30-45。',
          failoverEnabled: '首字超时自动换号',
          failoverEnabledHint: '开启后，首字超时会切换到另一个账号并仅重试当前请求。',
          initialAttemptTimeout: '第一次账号等待预算（秒）',
          initialAttemptTimeoutHint: '第一个账号首字前最多等待时间。0=使用总预算；建议 8-10。',
          maxSwitches: '最大换号次数',
          maxSwitchesHint: '首字输出前最多切换账号次数。0=不换号；建议 1。',
          penalizeAccount: '惩罚慢账号',
          penalizeAccountHint: '默认关闭：只换号，不惩罚账号。开启后慢账号可能被降权或隔离。',
          streamDataIntervalTimeout: 'SSE 静默超时（秒）',
          streamDataIntervalTimeoutHint: '流式输出过程中两个 SSE 事件之间允许的最大静默时间。0=关闭；建议 60。',
        },
        accountSchedulingThresholdsTitle: '平台账号自动暂停阈值',
        accountSchedulingThresholdsDescription: '当账号当前原生用量窗口（OpenAI Codex/Anthropic 会话，或 Grok 请求/Token 使用率）达到该百分比时，Sub2API 会临时从调度中移除它，直到窗口重置。使用 100 表示关闭。',
        accountSchedulingThresholdsGlobalHint: '该平台所有账号的系统默认值。单个账号仍可在账号编辑器中覆盖。',
        accountSchedulingThresholdsDisabledHint: '100 表示关闭平台自动暂停。1-99 表示使用率达到该百分比后暂停调度。',
        accountSchedulingThresholdsRangeHint: '整数 1-100（百分比）。仅适用于 OpenAI / Anthropic / Grok。'
      },
