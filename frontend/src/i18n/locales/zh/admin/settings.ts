export default {
    settings: {
      ...
      scheduling: {
        ...
        openaiFirstOutput: {
          title: 'OpenAI 首字 / 流超时',
          description: '这些值保存到数据库里，保存后立即生效。0 表示关闭。',
          firstOutputTimeout: '普通请求首字预算（秒）',
          firstOutputTimeoutHint: '首字前总预算。0=关闭；建议 20-25。',
          highEffortFirstOutputTimeout: '高耗时请求首字预算（秒）',
          highEffortFirstOutputTimeoutHint: 'high / xhigh / max 走更长预算。0=跟随普通预算；建议 30-45。',
          failoverEnabled: '首字超时自动换号',
          failoverEnabledHint: '开启后，首字前超时会换另一个账号重试；只影响当前请求。',
          initialAttemptTimeout: '第一次账号等待（秒）',
          initialAttemptTimeoutHint: '第一次账号首字等待上限。0=用总预算；建议 8-10。',
          maxSwitches: '最多换号次数',
          maxSwitchesHint: '首字前最多切换账号的次数。0=不换号；建议 1。',
          penalizeAccount: '首字超时处罚账号',
          penalizeAccountHint: '默认关闭，只换号不处罚账号。开启后会把慢账号记入熔断/降权。',
          streamDataIntervalTimeout: 'SSE 静默超时（秒）',
          streamDataIntervalTimeoutHint: '流式输出中两次 SSE 事件之间的最大静默时间。0=关闭；建议 60。',
        },
      },
