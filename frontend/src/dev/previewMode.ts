...
const previewUsageTimes = Array.from({ length: 20 }, (_, index) =>
  new Date(Date.UTC(2026, 8, 19, 20, 28, 1) - index * 73_000).toISOString()
)

export const previewUsageLogs = previewUsageTimes.map((createdAt, index) => ({
  id: 500 + index,
  user_id: previewUser.id,
  api_key_id: 101,
  account_id: null,
  request_id: `client:${String(395 - index * 13).padStart(3, '0')}:preview-${index}`,
  model: previewModels[index % previewModels.length].model,
  service_tier: null,
  reasoning_effort: index % 2 === 0 ? 'medium' : 'max',
  http_method: 'POST',
  status_code: 200,
  attempt_count: 1,
  inbound_endpoint: '/v1/responses',
  upstream_endpoint: '/v1/responses',
  group_id: previewGroups[0].id,
  subscription_id: 1,
  input_tokens: 4_700 + (index % 6) * 2_900,
  output_tokens: 1_500 + (index % 5) * 470,
  cache_creation_tokens: 7_500,
  cache_read_tokens: 70_400 - index * 1_350,
  ...
  actual_cost: 0.034175 + (index % 7) * 0.021027,
  rate_multiplier: 1,
  ...
  duration_ms: 59_170 - index * 1_370,
  first_token_ms: 24_790 - index * 710,
  image_count: 0,
  image_size: null,
