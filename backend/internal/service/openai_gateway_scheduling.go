...
func (s *OpenAIGatewayService) selectAccountForModelWithExclusions(ctx context.Context, groupID *int64, platform string, sessionHash string, requestedModel string, excludedIDs map[int64]struct{}, requireCompact bool, stickyAccountID int64, requiredCapability OpenAIEndpointCapability, preferLowUpstreamRate bool) (*Account, error) {
	...
	selected, compactBlocked, filterStats := s.selectBestAccount(ctx, groupID, platform, accounts, requestedModel, excludedIDs, requireCompact, requiredCapability, preferLowUpstreamRate, allowRuntimeBlocked)
...
func (s *OpenAIGatewayService) selectBestAccount(ctx context.Context, groupID *int64, platform string, accounts []Account, requestedModel string, excludedIDs map[int64]struct{}, requireCompact bool, requiredCapability OpenAIEndpointCapability, preferLowUpstreamRate bool, allowRuntimeBlocked bool) (*Account, bool, openAISelectionFilterStats) {
	...
	for i := range accounts {
		...
		fresh := s.resolveFreshSchedulableOpenAIAccountBeforeProfitWithRuntimeBlockPolicy(ctx, acc, platform, requestedModel, false, requiredCapability, allowRuntimeBlocked)
		if fresh == nil {
		...
		fresh = s.recheckSelectedOpenAIAccountFromDBBeforeProfitWithRuntimeBlockPolicy(ctx, fresh, groupID, platform, requestedModel, false, requiredCapability, allowRuntimeBlocked)
		if fresh == nil {
			filterStats.exclude("ineligible")
			continue
		}
...
func (s *OpenAIGatewayService) recheckSelectedOpenAIAccountFromDBWithRuntimeBlockPolicy(ctx context.Context, account *Account, groupID *int64, platform string, requestedModel string, requireCompact bool, requiredCapability OpenAIEndpointCapability, allowRuntimeBlocked bool) *Account {
	latest := s.recheckSelectedOpenAIAccountFromDBBeforeProfitWithRuntimeBlockPolicy(ctx, account, groupID, platform, requestedModel, requireCompact, requiredCapability, allowRuntimeBlocked)
	if latest == nil {
		return nil
	}
