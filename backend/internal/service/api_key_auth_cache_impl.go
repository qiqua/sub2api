...
const apiKeyAuthSnapshotVersion = 18 // v18: include API key auto routing, group reasoning effort policy, OpenAI group Live gate, and group profit control fields
...
func (s *APIKeyService) snapshotFromAPIKey(ctx context.Context, apiKey *APIKey) *APIKeyAuthSnapshot {
	...
	snapshot := &APIKeyAuthSnapshot{
		...
		RoutingMode:       NormalizeAPIKeyRoutingMode(apiKey.RoutingMode),
		AutoRouteGroupIDs: NormalizeAPIKeyAutoRouteGroupIDs(apiKey.AutoRouteGroupIDs),
		Name:              apiKey.Name,
...
func (s *APIKeyService) snapshotToAPIKey(key string, snapshot *APIKeyAuthSnapshot) *APIKey {
	...
	apiKey := &APIKey{
		...
		RoutingMode:       NormalizeAPIKeyRoutingMode(snapshot.RoutingMode),
		AutoRouteGroupIDs: NormalizeAPIKeyAutoRouteGroupIDs(snapshot.AutoRouteGroupIDs),
		Key:               key,
		Name:              snapshot.Name,
