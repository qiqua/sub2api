...
func TestAPIKeyService_RejectsV16AuthSnapshotBeforeMergedRoutingAndReasoningPolicy(t *testing.T) {
	svc := &APIKeyService{}

	apiKey, ok, err := svc.applyAuthCacheEntry("k-legacy-merged-auth-snapshot", &APIKeyAuthCacheEntry{
	...
	if ok {
		t.Fatal("expected v16 auth snapshot to be rejected after auto routing and reasoning policy snapshots were merged")
	}
	if apiKey != nil {
		t.Fatalf("expected no API key from stale snapshot, got %#v", apiKey)
	}
...
func TestAPIKeyService_RejectsV17AuthSnapshotBeforeOpenAILiveGate(t *testing.T) {
	svc := &APIKeyService{}

	apiKey, ok, err := svc.applyAuthCacheEntry("k-legacy-openai-live-gate", &APIKeyAuthCacheEntry{
	...
	if ok {
		t.Fatal("expected v17 auth snapshot to be rejected after OpenAI Live gate was added")
	}
	if apiKey != nil {
		t.Fatalf("expected no API key from stale snapshot, got %#v", apiKey)
	}
