		...
		if err != nil {
			...
			if errors.As(err, &failoverErr) {
				...
				if failoverErr.ShouldReportAccountScheduleFailure() {
					h.gatewayService.ReportOpenAIAccountScheduleResultEx(account.ID, grokMediaScheduleModel(account, routingModel, nil), service.OpenAIAccountScheduleReport{Success: false, StatusCode: failoverErr.StatusCode})
				}
